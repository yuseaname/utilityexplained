...
This example uses a typical heating-dominant home in the Northeast. Actual amounts vary by climate, fuel, and rate plan.
...
- **Deficit at settle-up** — if the plan under-collects, the balance must be paid at review or when service ends.
- **Not a discount** — you pay the same total annual charges as standard billing; there is no savings from the plan itself.
...
Treat the plan payment and actual account activity as separate information. Each bill should still be reviewed for billing days, usage, read status, rate-plan changes, credits, and one-time items. If the plan displays a balance or settlement amount, ask the utility to explain the calculation and the terms that apply.
