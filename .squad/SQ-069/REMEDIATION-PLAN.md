...
- [ ] P1.6 **C7 eight uncited claims**: source-or-soften in `average-utility-bills-by-state-2026:126` ×2, `07-gas-bill-too-high:160-161` ×2, `44-how-to-lower-water-bill:104-107` ×3, `05-how-to-lower-utility-bills:85` ×1 — *(mason + web)*
- [ ] P1.V Commander verification + `hugo` build + **commit `fix: SQ-069 phase 1 — critical fixes`**
...
- [ ] P3.3 **Brief 2** `duct-leaks-symptoms` rebuild (symptom list, DIY checklist, runtime math table) — *(mason)*
- [ ] P3.4 **Brief 3** `drought-surcharge` rebuild (named-utility drought-rate table, before/after delta) — *(mason)*
- [ ] P3.5 **Brief 4** `furnace-cold-air` spine-preserving rebuild (15-min check table, cost bands, 25C-ended note) — *(mason)*
...
- [ ] P4.1 **Alt-text template fix**: card-image alt = guide title in layouts (kills all 252) — *(executor)*
- [ ] P4.2 **Title trims**: worst ~30 front-matter titles >60 chars — *(mason)*
...
- [ ] P4.5 **Corrections log**: first two entries (SQ-067 DOE-link fix; page-46 Con Ed correction) on corrections.md — *(lieutenant)*
...
- [ ] P4.10 **Retrieval-date backfill** batch 3 (remaining ~14) + sources for 4 sourceless posts — *(mason + web)*
